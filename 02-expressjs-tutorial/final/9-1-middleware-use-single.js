...
app.use("/api", logger); // Now we don't want to run it for all, we can specify a prefix path, the logger will execute for those url which has prefix path of "/api"; Eg of such url is: "/api/products". We can use it for get method as well.
...
app.get("/api/products", (req, res) => {
  res.json([
    { productId: 1, name: "Alu" },
    { productId: 2, name: "Bhaji" },
  ]);
});
app.get("/api/items", (req, res) => {
  res.json([
    { productId: 1, name: "Ecommerce Project" },
    { productId: 2, name: "School Management" },
  ]);
});
